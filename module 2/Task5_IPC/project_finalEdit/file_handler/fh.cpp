...
int pipe_read(void)
{
    ...
    char pathBuffer[256];
    ssize_t pathCount = read(fd, pathBuffer, sizeof(pathBuffer));
    if (pathCount <= 0) {
        std::cerr << "Error reading path from the pipe." << std::endl;
        close(fd);
        return -1;
    }
    filePath = pathBuffer;
    ...
    std::cout << "Received value: " << value << " and Path: " << filePath << std::endl;
...
int sharedMem_write(void)
{
    ...
    if (value == 0) {
        // Open the file for reading using the received path
        ...
        FILE* ls_output = popen(("ls " + filePath).c_str(), "r");
